...
@section('content')
    ...
    <table id="list" class="responsive table-hover table-striped" data-sorting="true" data-paging="true"
           data-filtering="true">
        <thead>
        <tr>
                ...
                @if($value['status'] == 3)
                    <td>
                        <form method="POST" action="/approved-pdf">
                            ...
                            <input type="submit" class="btn btn-primary" name="Отправить на email"
                                   value="Отправить на email">
                        </form>
                    </td>
                @else
                    <td>
                        <form method="POST" action="/approved-pdf">
                            <input type="text" name="appID" value="{{ $value['AppID'] }}" style="display: none;">
                            <input type="submit" disabled class="btn btn-primary" name="Отправить на email"
                                   value="Отправить на email">
                        </form>
                    </td>
                @endif
