...
@section('content')
    ...
    <div>
        ...
        Курс на начало набития
        счета: {{ Form::label('currency', round($orders[0]['Kurs'], 2), ['id' => 'currency','class' => 'mylabel']) }}
        ...
        оплаты: {{ Form::label('typeInv', $orders[0]['CodeID3'], ['id' => 'typeInv','class' => 'mylabel', 'hidden' => 'hidden']) }}
        ...
        <table class="table table-hover table-responsive" style="width:100%;">
            ...
            @foreach($orders as $key => $value)
                <tr class="touch s{{ $value['status'] }}" style="background-color: {{ $value['color'] }}">
                    ...
                    <th class="Qty"><input value="{{ round($value['Qty'], 1) }}" class="quantity" type="number"
                                           autocorrect="off" pattern="\d+" novalidate/></th>
                    <th class="UM">{{ $value['UM'] }}</th>
                    <th><span style="width: 100%; height: 100%" class="btn m-price PriceMC" name="{{ $value['PLID'] }}"
                              id="m-price{{$key}}">{{ ceil(($value['PriceMC'] / 6) * 100) / 100 * 6 }}</span></th>
                    ...
                    @if(count($value['pl']) == 10)
                        ...
                        <th class="pl2" id="pl1" name="{{ $value['pl'][1]['PLID'] }}"
                            hidden="hidden">{{ ceil(($value['pl'][1]['PriceMC'] * $currency / 6) * 100) / 100 * 6 }}</th>
                        <th class="pl3" id="pl2" name="{{ $value['pl'][2]['PLID'] }}"
                            hidden="hidden">{{ ceil(($value['pl'][2]['PriceMC'] * $currency / 6) * 100) / 100 * 6 }}</th>
                        <th class="pl4" id="pl3" name="{{ $value['pl'][3]['PLID'] }}"
                            hidden="hidden">{{ ceil(($value['pl'][3]['PriceMC'] * $currency / 6) * 100) / 100 * 6 }}</th>
                        <th class="pl5" id="pl4" name="{{ $value['pl'][4]['PLID'] }}"
                            hidden="hidden">{{ ceil(($value['pl'][4]['PriceMC'] * $currency / 6) * 100) / 100 * 6 }}</th>
                        <th class="pl6" id="pl5" name="{{ $value['pl'][5]['PLID'] }}"
                            hidden="hidden">{{ ceil(($value['pl'][5]['PriceMC'] * $currency / 6) * 100) / 100 * 6 }}</th>
                        <th class="pl7" id="pl6" name="{{ $value['pl'][6]['PLID'] }}"
                            hidden="hidden">{{ ceil(($value['pl'][6]['PriceMC'] * $currency / 6) * 100) / 100 * 6 }}</th>
                        <th class="pl8" id="pl7" name="{{ $value['pl'][7]['PLID'] }}"
                            hidden="hidden">{{ ceil(($value['pl'][7]['PriceMC'] * $currency / 6) * 100) / 100 * 6 }}</th>
                        <th class="pl9" id="pl8" name="{{ $value['pl'][8]['PLID'] }}"
                            hidden="hidden">{{ ceil(($value['pl'][8]['PriceMC'] * $currency / 6) * 100) / 100 * 6 }}</th>
                        <th class="pl10" id="p19" name="{{ $value['pl'][9]['PLID'] }}"
                            hidden="hidden">{{ ceil(($value['pl'][9]['PriceMC'] * $currency / 6) * 100) / 100 * 6 }}</th>
                        <th class="pl0" id="pl110" name="" hidden="hidden"> -</th>
                    @elseif(count($value['pl']) == 11)
                        <th class="pl0" id="pl0" name="{{ $value['pl'][0]['PLID'] }}"
                            hidden="hidden">{{ ceil(($value['pl'][0]['PriceMC'] * $currency / 6) * 100) / 100 * 6 }}</th>
                        <th class="pl1" id="pl1" name="{{ $value['pl'][1]['PLID'] }}"
                            hidden="hidden">{{ ceil(($value['pl'][1]['PriceMC'] * $currency / 6) * 100) / 100 * 6 }}</th>
                        <th class="pl2" id="pl2" name="{{ $value['pl'][2]['PLID'] }}"
                            hidden="hidden">{{ ceil(($value['pl'][2]['PriceMC'] * $currency / 6) * 100) / 100 * 6 }}</th>
                        <th class="pl3" id="pl3" name="{{ $value['pl'][3]['PLID'] }}"
                            hidden="hidden">{{ ceil(($value['pl'][3]['PriceMC'] * $currency / 6) * 100) / 100 * 6 }}</th>
                        <th class="pl4" id="pl4" name="{{ $value['pl'][4]['PLID'] }}"
                            hidden="hidden">{{ ceil(($value['pl'][4]['PriceMC'] * $currency / 6) * 100) / 100 * 6 }}</th>
                        <th class="pl5" id="pl5" name="{{ $value['pl'][5]['PLID'] }}"
                            hidden="hidden">{{ ceil(($value['pl'][5]['PriceMC'] * $currency / 6) * 100) / 100 * 6 }}</th>
                        <th class="pl6" id="pl6" name="{{ $value['pl'][6]['PLID'] }}"
                            hidden="hidden">{{ ceil(($value['pl'][6]['PriceMC'] * $currency / 6) * 100) / 100 * 6 }}</th>
                        <th class="pl7" id="pl7" name="{{ $value['pl'][7]['PLID'] }}"
                            hidden="hidden">{{ ceil(($value['pl'][7]['PriceMC'] * $currency / 6) * 100) / 100 * 6 }}</th>
                        <th class="pl8" id="pl8" name="{{ $value['pl'][8]['PLID'] }}"
                            hidden="hidden">{{ ceil(($value['pl'][8]['PriceMC'] * $currency / 6) * 100) / 100 * 6 }}</th>
                        <th class="pl9" id="pl9" name="{{ $value['pl'][9]['PLID'] }}"
                            hidden="hidden">{{ ceil(($value['pl'][9]['PriceMC'] * $currency / 6) * 100) / 100 * 6 }}</th>
                        <th class="pl10" id="pl10" name="{{ $value['pl'][10]['PLID'] }}"
                            hidden="hidden">{{ ceil(($value['pl'][10]['PriceMC'] * $currency / 6) * 100) / 100 * 6 }}</th>
                    @endif
                </tr>
            @endforeach
            <tr id="tbody-pos"></tr>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="table table-responsive"
                         style="display: table; margin: 0 auto; text-align: center; border: 1px solid black; border-collapse: collapse;">
                        <div style="display: table-header-group;">
                            <div class="tr" style="display: table-row;">
                                ...
                                <div style="background-color: #ffff00; border: 1px solid black; display: table-cell;"
                                     class="td">111
                                </div>
                            </div>
                        </div>
                        <div style="display: table-row-group;">
                            <div class="tr" style="display: table-row;">
                                <div style="border: 1px solid black; display: table-cell;" class="btn price-val"
                                     id="p0m" name="101"></div>
                                <div style="border: 1px solid black; display: table-cell;" class="btn price-val"
                                     id="p1m" name="102"></div>
                                <div style="border: 1px solid black; display: table-cell;" class="btn price-val"
                                ...
                                <div style="border: 1px solid black; display: table-cell;" class="btn price-val"
                                     id="p9m" name="110"></div>
                                <div style="background-color: #ffff00; border: 1px solid black; display: table-cell;"
                                     class="btn price-val" id="p10m" name="111"></div>
                            </div>
                        </div>
                    </div>
    ...
    <div id="mModal" class="modal fade" role="dialog" hidden="hidden">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div id="table-modal" class="table table-responsive"
                         style="display: table; margin: 0 auto; text-align: center; border: 1px solid black; border-collapse: collapse;">
                        <div style="display: table-header-group;">
                            <div class="tr" style="display: table-row;">
                                <div style="border: 1px solid black; display: table-cell;" class="td">101</div>
                            ...
                            <div class="tr tr-val" style="display: table-row;">
                                <div style="border: 1px solid black; display: table-cell;" class="btn m-price-val"
                                     id="p00m" name="101"></div>
                                <div style="border: 1px solid black; display: table-cell;" class="btn m-price-val"
                                     id="p01m" name="102"></div>
                                <div style="border: 1px solid black; display: table-cell;" class="btn m-price-val"
                                     id="p02m" name="103"></div>
                                <div style="border: 1px solid black; display: table-cell;" class="btn m-price-val"
                                     id="p03m" name="104"></div>
                                <div style="border: 1px solid black; display: table-cell;" class="btn m-price-val"
                                     id="p04m" name="105"></div>
                                <div style="border: 1px solid black; display: table-cell;" class="btn m-price-val"
                                     id="p05m" name="106"></div>
                                <div style="border: 1px solid black; display: table-cell;" class="btn m-price-val"
                                     id="p06m" name="107"></div>
                                <div style="border: 1px solid black; display: table-cell;" class="btn m-price-val"
                                     id="p07m" name="108"></div>
                                <div style="border: 1px solid black; display: table-cell;" class="btn m-price-val"
                                     id="p08m" name="109"></div>
                                <div style="border: 1px solid black; display: table-cell;" class="btn m-price-val"
                                     id="p09m" name="110"></div>
                                <div style="background-color: #ffff00; border: 1px solid black; display: table-cell;"
                                     class="btn m-price-val" id="p010m" name="111"></div>
                            </div>
                        </div>
                    </div>
                    <br/>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" data-dismiss="modal" id="m-modal-close">Закрыть
                    </button>
