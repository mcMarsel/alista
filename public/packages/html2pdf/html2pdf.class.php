...
if (!defined('__CLASS_HTML2PDF__')) {
	...
	class HTML2PDF
	{
		...
		public function writeHTML($html, $debugVue = false)
		{
			...
			if (preg_match('/<body/isU', $html)) $html = $this->getHtmlFromPage($html);
		...
		public function getHtmlFromPage($html)
		{
			...
			foreach ($match[0] as $src) $content = $src . '</link>' . $content;

			// extract the css style tags
			preg_match_all('/<style[^>]*>(.*)<\/style[^>]*>/isU', $html, $match);
			foreach ($match[0] as $src) $content = $src . $content;
...
' . $content . '
		...
		protected function _setNewPage($format = null, $orientation = '', $background = null, $curr = null, $resetPageNumber = false)
		{
			...
			if (!$this->_subPart && !$this->_isSubPart) {
				if (is_array($this->_background)) {
					...
					if (isset($this->_background['img']) && $this->_background['img']) $this->pdf->Image($this->_background['img'], $this->_background['posX'], $this->_background['posY'], $this->_background['width']);
				}
		...
		protected function _getMargins($y)
		{
			...
			foreach ($this->_pageMarges as $mY => $mX) if ($mY <= $y) $x = $mX;
		...
		protected function _setNewPositionForNewLine($curr = null)
		{
			...
			if ($this->parsingCss->value['text-align'] == 'center') $this->pdf->setX(($rx + $this->pdf->getX() - $w) * 0.5 - 0.01); else if ($this->parsingCss->value['text-align'] == 'right') $this->pdf->setX($rx - $w - 0.01); else
				$this->pdf->setX($lx);
		...
		protected function _listeAddLevel($type = 'ul', $style = '', $img = null)
		{
			...
			if (!$style) {
				if ($type == 'ul') $style = 'disc'; else                $style = 'decimal';
			}
		...
		{
			if ($this->_subPart || $this->_isSubPart || !$this->_currentH || $this->_currentH < $h) return 0;
		...
		{
			if ($h) {
				if (($this->pdf->getY() + $h < $this->pdf->getH() - $this->pdf->getbMargin()) || $this->_isInOverflow || $this->_isInFooter) $this->_setNewLine($h, $curr); else
		...
		protected function _drawRectangle($x, $y, $w, $h, $border, $padding, $margin, $background)
		{
			...
			if ($background['image']) {
				...
				if (count($imageInfos) < 2) {
					...
					if ($iRepeat[0]) $iPosition[0] = $bX; else if (preg_match('/^([-]?[0-9\.]+)%/isU', $iPosition[0], $match)) $iPosition[0] = $bX + $match[1] * ($bW - $imageWidth) / 100; else $iPosition[0] = $bX + $iPosition[0];

					if ($iRepeat[1]) $iPosition[1] = $bY; else if (preg_match('/^([-]?[0-9\.]+)%/isU', $iPosition[1], $match)) $iPosition[1] = $bY + $match[1] * ($bH - $imageHeight) / 100; else $iPosition[1] = $bY + $iPosition[1];
		...
		protected function _drawCurve($pt, $color)
		{
			...
			if (count($pt) == 10) $this->pdf->drawCurve($pt[0], $pt[1], $pt[2], $pt[3], $pt[4], $pt[5], $pt[6], $pt[7], $pt[8], $pt[9]); else
		...
		protected function _tag_open_NOBREAK($param)
		{
			...
			if ($sub->_maxY < ($this->pdf->getH() - $this->pdf->gettMargin() - $this->pdf->getbMargin()) && $y + $sub->_maxY >= ($this->pdf->getH() - $this->pdf->getbMargin())) {
				$this->_setNewPage();
			}
		...
		protected function _tag_open_DIV($param, $other = 'div')
		{
			...
			if (!$this->parsingCss->value['position']) {
				if ($w < ($this->pdf->getW() - $this->pdf->getlMargin() - $this->pdf->getrMargin()) && $this->pdf->getX() + $w >= ($this->pdf->getW() - $this->pdf->getrMargin())) $this->_tag_open_BR(array());

				if (($h < ($this->pdf->getH() - $this->pdf->gettMargin() - $this->pdf->getbMargin())) && ($this->pdf->getY() + $h >= ($this->pdf->getH() - $this->pdf->getbMargin())) && !$this->_isInOverflow) $this->_setNewPage();

				$old = $this->parsingCss->getOldValues();
				$parentWidth = $old['width'] ? $old['width'] : $this->pdf->getW() - $this->pdf->getlMargin() - $this->pdf->getrMargin();

				if ($parentWidth > $w) {
					if ($alignObject == 'center') $this->pdf->setX($this->pdf->getX() + ($parentWidth - $w) * 0.5); else if ($alignObject == 'right') $this->pdf->setX($this->pdf->getX() + $parentWidth - $w);
				}
			...
			for ($tempPos = $this->_parsePos + 1; $tempPos < count($this->parsingHtml->code); $tempPos++) {
				...
				if ($action['name'] == 'legend' && !$action['close']) {
					...
					for ($sub->_parsePos = 0; $sub->_parsePos < count($sub->parsingHtml->code); $sub->_parsePos++) {
						...
						if ($action['name'] == 'legend' && $action['close']) break;
					}
		...
		protected function _tag_close_DIV($param, $other = 'div')
		{
			...
			if ($this->parsingCss->value['rotate']) $this->pdf->stopTransform();
		...
		protected function _tag_open_BARCODE($param)
		{
			...
			if ($this->_testIsDeprecated && (isset($param['bar_h']) || isset($param['bar_w']))) throw new HTML2PDF_exception(9, array('BARCODE', 'bar_h, bar_w'));
		...
		{
			if ($this->_testIsDeprecated && (isset($param['size']) || isset($param['noborder']))) throw new HTML2PDF_exception(9, array('QRCODE', 'size, noborder'));
		...
		protected function _tag_open_WRITE($param)
		{
			...
			if ($this->parsingCss->value['text-transform'] != 'none') {
				if ($this->parsingCss->value['text-transform'] == 'capitalize') $txt = ucwords($txt); else if ($this->parsingCss->value['text-transform'] == 'uppercase') $txt = strtoupper($txt); else if ($this->parsingCss->value['text-transform'] == 'lowercase') $txt = strtolower($txt);
			}
		...
		protected function _tag_open_LI($param)
		{
			...
			if ($this->_subPart) {
				...
				if (isset($paramPUCE['src'])) $this->_tag_open_IMG($paramPUCE); else                            $this->_tag_open_WRITE($paramPUCE);
		...
		protected function _tag_open_FORM($param)
		{
			...
			$this->pdf->setFormDefaultProp(array('lineWidth' => 1, 'borderStyle' => 'solid', 'fillColor' => array(220, 220, 255), 'strokeColor' => array(128, 128, 200)));
			...
			if ($this->_subPart) {
				...
				foreach ($lst as $mode) {
					...
					foreach (HTML2PDF::$_tables[$param['num']][$mode]['tr'] as $tr) {
						...
						$h = 0;
						for ($i = 0; $i < count(HTML2PDF::$_tables[$param['num']]['cases'][$tr]); $i++) if (HTML2PDF::$_tables[$param['num']]['cases'][$tr][$i]['rowspan'] == 1) $h = max($h, HTML2PDF::$_tables[$param['num']]['cases'][$tr][$i]['h']);
				...
				if ($parentWidth > $w) {
					if (HTML2PDF::$_tables[$param['num']]['align'] == 'center') $x = $x + ($parentWidth - $w) * 0.5; else if (HTML2PDF::$_tables[$param['num']]['align'] == 'right') $x = $x + $parentWidth - $w;
				...
				for ($k = 0; $k < count(HTML2PDF::$_tables[$param['num']]['cases']); $k++) {
					...
					for ($i = 0; $i < count(HTML2PDF::$_tables[$param['num']]['cases'][$k]); $i++) {
						...
						if (HTML2PDF::$_tables[$param['num']]['cases'][$k][$i]['rowspan'] == 1) $th = max($th, HTML2PDF::$_tables[$param['num']]['cases'][$k][$i]['h']);
					}
				...
				if (count(HTML2PDF::$_tables[$param['num']]['height']) > 1) $y = $this->_margeTop + HTML2PDF::$_tables[$param['num']]['height'][count(HTML2PDF::$_tables[$param['num']]['height']) - 1]; else if (count(HTML2PDF::$_tables[$param['num']]['height']) == 1) $y = HTML2PDF::$_tables[$param['num']]['curr_y'] + HTML2PDF::$_tables[$param['num']]['height'][count(HTML2PDF::$_tables[$param['num']]['height']) - 1]; else
					$y = HTML2PDF::$_tables[$param['num']]['curr_y'];
		...
		protected function _tag_open_COL($param)
		{
			...
			for ($k = 0; $k < $span; $k++) HTML2PDF::$_tables[$param['num']]['cols'][] = $param;
		}
		...
		protected function _tag_open_TR($param, $other = 'tr')
		{
			...
			if (!$this->_subPart) {
				...
				if (!isset(HTML2PDF::$_tables[$param['num']]['corr'][HTML2PDF::$_tables[$param['num']]['corr_y']])) HTML2PDF::$_tables[$param['num']]['corr'][HTML2PDF::$_tables[$param['num']]['corr_y']] = array();

				HTML2PDF::$_tables[$param['num']]['corr_x'] = 0;
				while (isset(HTML2PDF::$_tables[$param['num']]['corr'][HTML2PDF::$_tables[$param['num']]['corr_y']][HTML2PDF::$_tables[$param['num']]['corr_x']])) HTML2PDF::$_tables[$param['num']]['corr_x']++;
			}
		...
		protected function _tag_open_TD($param, $other = 'td')
		{
				...
				if (isset(HTML2PDF::$_tables[$param['num']]['cols'][$numCol])) {
					...
					if (count($colParam['style']['width'])) {
						...
						foreach ($colParam['style']['width'] as $width) {
							if (substr($total, -1) == '%' && substr($width, -1) == '%') $total = (str_replace('%', '', $total) + str_replace('%', '', $width)) . '%'; else
			...
			if (in_array($other, array('td', 'th'))) {
				...
				if ($old && ($old['color'] || $old['image'])) $legacy['background'] = $old;
			...
			if ($collapse) {
				if (!$this->_subPart) {
					if ((HTML2PDF::$_tables[$param['num']]['tr_curr'] > 1 && !HTML2PDF::$_tables[$param['num']]['new_page']) || (!$this->_isInThead && count(HTML2PDF::$_tables[$param['num']]['thead']['code']))) {
