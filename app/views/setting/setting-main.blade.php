...
<div class="tab-content" >
    <div class="active tab-pane fade in" id="prod_settings">
        <h1><small>Редактирование и добавление групп товара</small></h1>
        <br/>
                ...
                @foreach($pgr3Obj as $key => $value)
                    <tr>
                        @if($value['status'] ==  1)
                            ...
                            <td>
                                <input type="submit" class="btn btn-primary hideGr3" name="Скрыть" value="Скрыть" id="{{ $value['PGrID3'] }}">
                            </td>
                        @else
                            <td>{{ $value['PGrName3'] }}</td>
                            <td>
                                <input type="submit" class="btn btn-primary showGr3" name="Показать" value="Показать" id="{{ $value['PGrID3'] }}">
                            </td>
                        @endif
                    </tr>
                @endforeach
    ...
    <div class="tab-pane fade in" id="user_settings">
        <h1><small>Управление пользователями</small></h1>
        <br/>
                ...
                @foreach($users as $key => $value)
                    <tr>
                        @if($value['status'] ==  1)
                            ...
                            <td>{{ $value['EmpID'] }}</td>
                            <td>
                                <input type="button" status="0" class="btn btn-primary degrade" name="Розжаловать" value="Розжаловать" id="{{ $value['id'] }}">
                            </td>
                            <td>
                                <input type="button" class="btn btn-primary edit" name="Редактировать" value="Редактировать" id="{{ $value['id'] }}">
                            </td>
                            <td>
                                <input type="button" class="btn btn-primary del_user" name="Удалить" value="Удалить" id="{{ $value['id'] }}">
                            </td>
                        @else
                            <td>{{ $value['EmpName'] }}</td>
                            <td>{{ $value['EmpID'] }}</td>
                            <td>
                                <input type="button" status="1" class="btn btn-primary increase" name="Повысить" value="Повысить" id="{{ $value['id'] }}">
                            </td>
                            <td>
                                <input type="button" class="btn btn-primary edit" name="Редактировать" value="Редактировать" id="{{ $value['id'] }}">
                            </td>
    ...
    <div class="tab-pane fade in" id="head_settings">
            <h1><small>Управление руководителями</small></h1>
            <br/>
                ...
                <tbody>
                    @foreach($head as $key => $value)
                        <tr>
                            ...
                            <td>{{ $value['MinPLID'] }}</td>
                            <td>
                                <input type="button" class="btn btn-primary degrade_head" name="Розжаловать" value="Розжаловать" id="{{ $value['id'] }}">
                            </td>
                        </tr>
                    @endforeach
                </tbody>
    ...
    <div class="tab-pane fade in" id="currency_settings">
        <h1><small>Управление курсом</small></h1>
        <br/>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                <button type="button" class="btn btn-default" data-dismiss="modal" id="modal-done">Сохранить</button>
            </div>
        </div>
    </div>
</div>
