...
{
	const HTML_TAB = '        ';        // HTML code to parse
	public $code = array();         // table number
	protected $_html = '';         // table level
	protected $_num = 0;        // encoding
	protected $_level = 0;   // parsed HTML codfe
	protected $_encoding = '';
	...
	public function parse()
	{
		...
		$tagPreBr = array('name' => 'br', 'close' => false, 'param' => array('style' => array(), 'num' => 0));
		...
		foreach ($tmp as $part) {
			...
			if ($part[0] == 'code') {
				...
				if ($res) {
					...
					if (!in_array($res['name'], $tagsNotClosed)) {
						...
						if ($res['close']) {
							// HTML validation
							if (count($parents) < 1) throw new HTML2PDF_exception(3, $res['name'], $this->getHtmlErrorCode($res['html_pos'])); else if ($parents[count($parents) - 1] != $res['name']) throw new HTML2PDF_exception(4, $parents, $this->getHtmlErrorCode($res['html_pos'])); else
			...
			if ($part[0] == 'txt') {
				...
				if (!$tagPreIn) {
					// save the action
					$actions[] = array('name' => 'write', 'close' => false, 'param' => array('txt' => $this->_prepareTxt($part[1])),);
					...
					foreach ($part[1] as $k => $txt) {
						...
						// save the action
						$actions[] = array('name' => 'write', 'close' => false, 'param' => array('txt' => $this->_prepareTxt($txt, false)),);
					}
				}
			}
		}
		...
		for ($k = 0; $k < $nb; $k++) {
			...
			if ($actions[$k]['name'] == 'write') {
				...
				if ($k > 0 && in_array($actions[$k - 1]['name'], $tagsToClean)) $actions[$k]['param']['txt'] = ltrim($actions[$k]['param']['txt']);

				// if the tag after the text is a tag to clean => rtrim on the text
				if ($k < $nb - 1 && in_array($actions[$k + 1]['name'], $tagsToClean)) $actions[$k]['param']['txt'] = rtrim($actions[$k]['param']['txt']);

				// if the text is empty => remove the action
				if (!strlen($actions[$k]['param']['txt'])) unset($actions[$k]);
			}
		}
	...
	protected function _analiseCode($code)
	{
		...
		for ($k = 0; $k < count($match[0]); $k++) $param[trim(strtolower($match[1][$k]))] = trim($match[2][$k]);
		...
		if ($border !== null) {
			if ($border) $border = 'border: solid ' . $border . ' ' . $color; else            $border = 'border: none';
