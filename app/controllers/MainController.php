...
class MainController extends BaseController
{
	...
	{
		$n = 'krutijk';
		$data = ['customer' => 'John Smith', 'url' => 'http://laravel.com'];
	...
	public function index()
	{
		...
		if (Auth::getUser()->status == 0) {
            ...
            $pgr3Obj = \ProdGr3::where('status', '=', 1)->orderBy('PGrName3', 'asc')->get(['PGrID3', 'PGrName3'])->toArray();
            $pgrName3 = [];
            $pgrID3 = [];
			...
			$comps = \Comps::/*whereIn('EmpID', $empsID)->*/
			orderBy('CompName', 'asc')->get(['CompName'])->toArray();
            $pgr3Obj = \ProdGr3::where('status', '=', 1)->orderBy('PGrName3', 'asc')->get(['PGrID3', 'PGrName3'])->toArray();
			$pgrName3 = [];
			$pgrID3 = [];
			foreach ($pgr3Obj as $key => $value) {
